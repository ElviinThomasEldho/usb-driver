...
static int pause_thread(void *data) {
    ...
    if (pid == -1) {
        printk(KERN_INFO "Pause: No PID provided\n");
        return -EINVAL;
    }
...
static int resume_thread(void *data) {
    struct task_struct *task_to_pause;
    struct pid *pid_struct;
    ...
    while (!kthread_should_stop()) {
        ...
        if (ret < 0) {
            ...
            if(strstr(buffer, "Object Detected") != NULL && paused == 0) {            
                printk(KERN_INFO "Initializing pause\n");
                task = kthread_run(pause_thread, NULL, "pause_thread");
                if (IS_ERR(task)) {
                    printk(KERN_ALERT "Failed to create thread\n");
                }               
            } else if (strstr(buffer, "Object Not Detected") != NULL && paused == 1) {    
                printk(KERN_INFO "Initializing resume\n");
                task = kthread_run(resume_thread, NULL, "resume_thread");
                if (IS_ERR(task)) {
                    printk(KERN_ALERT "Failed to create thread\n");
                }         
            }                   
